...
description: 把 Fruugo 工作流部署到 OpenClaw `ecom` agent。适用于刚下载这个 skill 仓库的新机器，通过脚本把标准 Fruugo workflow 段落注入到 `~/.openclaw/workspaces/ecom/AGENTS.md`，让 Xiaolongxia 能稳定执行这个 skill 仓库中的 `run_fruugo_workflow.py`。
---
...
把标准 Fruugo workflow 注入 OpenClaw `ecom` agent 的 `AGENTS.md`，让 Feishu/Xiaolongxia 触发时更稳定，并且直接调用这个 skill 仓库内置的脚本和默认 assets/runtime。
